...
def test_bake_with_console_script_main(cookies):
    context = {'command_line_interface': 'click'}
    result = cookies.bake(extra_context=context)
    project_path, project_slug, project_dir = project_info(result)
    ...
    if sys.version_info >= (3, 5):
        ...
        main = file_loader(module_name, module_path).load_module()
    else:
        main = imp.load_source(module_name, module_path)
    runner = CliRunner()
    noarg_result = runner.invoke(main.main)
